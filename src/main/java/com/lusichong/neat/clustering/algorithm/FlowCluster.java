...
import com.lusichong.neat.clustering.base.MoPoint;
    ...
    public PointsOnSeg chooseMergeNeighbor(RoadMap rm, PointsOnSeg endRouteSeg,
        ...
        for (int i = 0; i < fNeighborhoodSize; i++) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (maxFactors[j] < neighborAbsWeights[i][j])
                    maxFactors[j] = neighborAbsWeights[i][j];//for adaptive weights
            }
        }
        for (int j = 0; j < 3; j++) {
            if (denominators[j] == 0) {
...
//          maxFactors[1] / denominators[1], maxFactors[2] / denominators[2]);
    ...
    public PointsOnSeg[] concatenateJuncs(RoadMap rm,
        ...
        if (candFront.size() != 0) {
            if (wq == 1 && khops >= 0)
                ps[0] = chooseMaxFlowNeighbor(rm, flowClus.getFirst(), candFront, posList, khops);//new for look for/backward
            else ps[0] = chooseMergeNeighbor(rm, flowClus.getFirst(), candFront, wq, wk, wv);

        }
        if (candEnd.size() != 0) {
            if (wq == 1 && khops >= 0)
                ps[1] = chooseMaxFlowNeighbor(rm, flowClus.getLast(), candEnd, posList, khops);//new for look for/backward
